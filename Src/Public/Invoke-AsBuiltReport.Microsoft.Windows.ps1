function Invoke-AsBuiltReport.Microsoft.WindowsServer {
    ...
    .LINK
        https://github.com/AsBuiltReport/AsBuiltReport.Microsoft.WindowsServer
    #>
    ...
    foreach ($System in $Target) {
        Section -Style Heading1 $System {
            ...
            $script:HostCPU = Get-WmiObject -Class Win32_Processor -ComputerName $System -Credential $Credential
            $script:HostComputer = Get-WmiObject -Class Win32_ComputerSystem -ComputerName $System -Credential $Credential
            $script:HostBIOS = Get-WmiObject -Class Win32_Bios -ComputerName $System -Credential $Credential
            $script:HostLicense = Invoke-Command -Session $TempPssSession { Get-WmiObject -Query 'Select * from SoftwareLicensingProduct' | Where-Object { $_.LicenseStatus -eq 1 } }
            #Host Hardware
            ...
            Section -Style Heading2 'Host OS' {
                ...
                Section -Style Heading3 'Roles and Features' {
                    ...
                    $HostRolesAndFeatures = Get-WindowsFeature -ComputerName $System | Where-Object { $_.Installed -eq $True }
